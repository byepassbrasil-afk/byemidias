-- =====================================================
-- ByeMidias Digital Signage CMS — COMPLETE DATABASE
...
-- Trigger to create profile on signup (function only, trigger created later)
...
-- =====================================================
-- 21. PARTNER ACCESS (simple login, no OAuth)
-- =====================================================
...
-- Content version increment on playlist/campaign changes
...
-- Auth user created -> create profile
...
-- Organization created -> create subscription
...
CREATE POLICY "org_admin_manage_units" ON units
    FOR ALL USING (organization_id = auth.user_organization_id() AND auth.user_role() IN ('admin', 'manager'));
...
CREATE POLICY "org_admin_manage_devices" ON devices
    FOR ALL USING (organization_id = auth.user_organization_id() AND auth.user_role() IN ('admin', 'manager', 'operator'));
...
CREATE POLICY "org_member_read_playlist_items" ON playlist_items
    FOR SELECT USING (playlist_id IN (SELECT id FROM playlists WHERE organization_id = auth.user_organization_id()));
CREATE POLICY "org_operator_manage_playlist_items" ON playlist_items
    FOR ALL USING (playlist_id IN (SELECT id FROM playlists WHERE organization_id = auth.user_organization_id()) AND auth.user_role() IN ('admin', 'manager', 'operator'));
...
CREATE POLICY "org_member_read_campaign_targets" ON campaign_targets
    FOR SELECT USING (campaign_id IN (SELECT id FROM campaigns WHERE organization_id = auth.user_organization_id()));
CREATE POLICY "org_operator_manage_campaign_targets" ON campaign_targets
    FOR ALL USING (campaign_id IN (SELECT id FROM campaigns WHERE organization_id = auth.user_organization_id()) AND auth.user_role() IN ('admin', 'manager', 'operator'));
...
CREATE POLICY "org_member_read_heartbeats" ON device_heartbeats
    FOR SELECT USING (device_id IN (SELECT id FROM devices WHERE organization_id = auth.user_organization_id()));
...
CREATE POLICY "org_admin_manage_device_commands" ON device_commands
    FOR INSERT WITH CHECK (device_id IN (SELECT id FROM devices WHERE organization_id = auth.user_organization_id()) AND auth.user_role() IN ('admin', 'manager'));
...
CREATE POLICY "org_admin_read_audit_logs" ON audit_logs
    FOR SELECT USING (organization_id = auth.user_organization_id() AND auth.user_role() IN ('admin', 'manager'));

-- SUBSCRIPTIONS
CREATE POLICY "super_admin_all_subscriptions" ON subscriptions
    FOR ALL USING (auth.user_role() = 'super_admin');
CREATE POLICY "org_admin_read_subscriptions" ON subscriptions
    FOR SELECT USING (organization_id = auth.user_organization_id() AND auth.user_role() IN ('admin', 'super_admin'));
...
CREATE POLICY "org_admin_read_white_label" ON white_label_settings
    FOR SELECT USING (organization_id = auth.user_organization_id() AND auth.user_role() = 'admin');
CREATE POLICY "org_admin_manage_white_label" ON white_label_settings
    FOR ALL USING (organization_id = auth.user_organization_id() AND auth.user_role() = 'admin');
...
CREATE POLICY "org_admin_manage_partner_devices" ON partner_devices
    FOR ALL USING (partner_access_id IN (SELECT id FROM partner_access WHERE organization_id = auth.user_organization_id()) AND auth.user_role() IN ('admin', 'manager'));
...
-- =====================================================
-- SEED: Default super admin (run AFTER creating user)
-- =====================================================
-- INSERT INTO profiles (id, organization_id, full_name, role, status)
-- VALUES ('YOUR-AUTH-USER-UUID-HERE', NULL, 'Super Admin', 'super_admin', 'active');

-- =====================================================
-- STORAGE BUCKETS (create in Supabase Dashboard)
-- =====================================================
-- Bucket: media (public, 500MB limit)
-- Bucket: templates (private, 10MB limit)
-- Bucket: branding (public, 5MB limit)

-- =====================================================
-- DONE! 23 tables, all RLS, all triggers, all functions
-- =====================================================
