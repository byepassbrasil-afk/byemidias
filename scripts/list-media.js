...
(async () => {
  ...
  const items = await pb.collection('media').getList(1, 50);
  console.log('Total:', items.items.length);
  for (const i of items.items) {
    console.log(' -', JSON.stringify({id: i.id.slice(0,8), name: i.name, type: i.type, url: (i.url || '').slice(0,60), status: i.status, duration: i.duration}));
  }
})();
